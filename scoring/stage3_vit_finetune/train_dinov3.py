#!/usr/bin/env python3
"""
Fine-tune DINOv3 ViT-B/16 for e-commerce image aesthetic scoring.

Backbone: facebook/dinov3-vitb16-pretrain-lvd1689m (HuggingFace transformers).
Pretrained on LVD-1689M (1.69B images) -- ~12x the scale of DINOv2.
...
train_transform = transforms.Compose([
    ScaleImage(332),                    # downscale only if > 332px
    PadToSquare(fill=255),              # white-pad to square (preserves product AR)
...
def main(args):
    ...
    criterion = nn.SmoothL1Loss()  # Huber: robust to noisy LLM labels
    scheduler = CosineAnnealingLR(optimizer, T_max=args.epochs)

    CKPT_DIR.mkdir(exist_ok=True)
    best_ckpt = CKPT_DIR / "dinov3_vitb16_best.pt"

    mlflow.set_experiment("dinov3_aesthetic_scorer")
    with mlflow.start_run(run_name=f"dinov3-ecom_lr{args.lr}_bs{args.batch_size}"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DINOv3 aesthetic scorer fine-tuning")
    parser.add_argument("--pretrained",       default="facebook/dinov3-vitb16-pretrain-lvd1689m",
                        help="HuggingFace model ID, e.g. facebook/dinov3-vitb16-pretrain-lvd1689m")
